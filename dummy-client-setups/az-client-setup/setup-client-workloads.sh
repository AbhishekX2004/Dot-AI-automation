#!/usr/bin/env bash
# =============================================================================
# setup-client-workloads.sh — Populate a Client AKS Cluster with Demo Resources
# =============================================================================
# Usage:
#   ./setup-client-workloads.sh [--cluster-name NAME] [--resource-group RG]
#
...
#   1. Updates kubeconfig for the client AKS cluster
...
#
# Prerequisites: az CLI, kubectl, terraform
# =============================================================================
set -euo pipefail
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    *)                error "Unknown flag: $1. Usage: $0 [--cluster-name NAME] [--resource-group RG]" ;;
  esac
done
...
if [[ -z "$RESOURCE_GROUP" ]]; then
  log "No --resource-group provided, reading from terraform output..."
  RESOURCE_GROUP=$(terraform output -raw resource_group_name 2>/dev/null) \
    || error "Could not read resource_group_name from terraform output. Pass --resource-group explicitly."
fi
